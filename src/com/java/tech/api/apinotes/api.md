...
### Different Types of Authentications in Java RestAssured. 
...
8. Custom Token (JWT – JSON Web Token)
...
public class SerializationTest extends BaseTest {
    ...
    public void testPostWithSerialization() {
        // 🔹 Build Payload in Java (NO JSON hardcoding)
        ...
        // 🔹 Send POST request with POJO (RestAssured will serialize → JSON)
        Response response =
                given()
...
public class SerializationDeserializationTest extends BaseTest {
    ...
    public void testPostWithSerDe() {
        ...
        // 🔹 2. Send POST Request (Serialization happens here)
        Response response =
                given()
                    .contentType(ContentType.JSON)
                    .body(payload)
        ...
        // 🔹 3. Deserialize Response JSON → POJO
...
101 Switching Protocols → Used for WebSockets / protocol upgrade.
...
200 OK
👉 Standard response for successful GET / PUT requests.
Example: Fetching user details returns 200.

201 Created
👉 Resource successfully created (mostly after POST).
Example: Creating a new product returns 201.

202 Accepted
👉 Request accepted but processing is async.
Example: Uploading a file for background processing.

204 No Content
👉 Success, but no body in response.
...
304 Not Modified → Used with caching (resource hasn’t changed).
...
400 Bad Request
👉 Invalid request payload / query params.
...
👉 Rate limiting (too many API calls in short time).
...
Example: Load balancer can’t reach app server.
...
given()
    .when()
        .get("/users/101")
    .then()
        .statusCode(200);  // ✅ validating response code


👉 Quick real-world mapping for clarity:

200 → GET user details successful
...
403 → Normal user hitting admin API

404 → User not found

500 → Backend code issue
...
🔹 9. Difference between Hard Assertions vs Soft Assertions in API tests?

Hard → stops test execution after first failure (assertThat).

Soft → collects all failures, reports at the end (SoftAssert in TestNG).

```
> ### Design Patterns of Test Automations.
...
🔹 Real-Life Example in Automation

In Selenium/Rest Assured frameworks, we don’t want multiple browser drivers or RestAssured configs unnecessarily. Instead, we use a singleton class to manage them.
...
```
🔹 What is the Builder Design Pattern?(Fluent design pattern also same)
...
- Write test cases in plain English (Gherkin syntax) → understandable by BA, Dev, QA.
(Cucumber + Gherkin):
Feature: Login
...
1. Install plugin: Manage Jenkins → Plugins → Available → Allure Jenkins Plugin (install + restart if needed).

2. Tool config: Manage Jenkins → Tools → Allure Commandline → “Add Allure Commandline”
Name it e.g. Allure2
Choose “Install automatically” or point to an installed path on the agent.
You don’t run allure serve in CI. The plugin will publish a permanent link on the build page.

- What you’ll see in Jenkins
On every build page, “Allure Report” link in the left sidebar.
...
- Switching on a light:
  If the light is already ON, pressing the "ON" button again doesn’t change anything.
...
No matter how many times setFlagTrue() is called → final state is the same (flag = true).
...
GET → Idempotent (fetching the same resource doesn’t change anything).

PUT → Idempotent (replacing a resource with the same value multiple times has the same effect).

DELETE → Idempotent (deleting the same resource multiple times still results in "not there").

POST → Usually NOT idempotent (each call creates a new resource).

- Why important?
Retry mechanisms: In distributed systems, retries happen (network failures, timeouts).
If your operation is idempotent, retries won’t cause wrong data.
